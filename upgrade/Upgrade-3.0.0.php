...
function upgrade_module_3_0_0($module)
{
    ...
    if (EnvoimoinscherModel::getConfig('EMC_USER') == '') {
        Configuration::updateValue('EMC_USER', 0);
    }
    if (EnvoimoinscherModel::getConfig('EMC_MSG') == '') {
        Configuration::updateValue('EMC_MSG', 'La plate-forme d\'expéditions est actuellement indisponible');
    }
    if (EnvoimoinscherModel::getConfig('EMC_MASS') == '') {
        Configuration::updateValue('EMC_MASS', EnvoimoinscherModel::WITH_CHECK);
    }
    if (EnvoimoinscherModel::getConfig('EMC_TRACK_MODE') == '') {
        Configuration::updateValue('EMC_TRACK_MODE', false);
    }
    if (EnvoimoinscherModel::getConfig('EMC_ASSU') == '') {
        Configuration::updateValue('EMC_ASSU', '');
    }
    if (EnvoimoinscherModel::getConfig('EMC_INDI') == '') {
        Configuration::updateValue('EMC_INDI', true);
    }
    if (EnvoimoinscherModel::getConfig('EMC_MULTIPARCEL') == '') {
        Configuration::updateValue('EMC_MULTIPARCEL', false);
    }
    if (EnvoimoinscherModel::getConfig('EMC_PICKUP_J1') == '') {
        Configuration::updateValue('EMC_PICKUP_J1', '2');
    }
    if (EnvoimoinscherModel::getConfig('EMC_PICKUP_F1') == '') {
        Configuration::updateValue('EMC_PICKUP_F1', '0');
    }
    if (EnvoimoinscherModel::getConfig('EMC_PICKUP_T1') == '') {
        Configuration::updateValue('EMC_PICKUP_T1', '17');
    }
    if (EnvoimoinscherModel::getConfig('EMC_PICKUP_J2') == '') {
        Configuration::updateValue('EMC_PICKUP_J2', '3');
    }
    if (EnvoimoinscherModel::getConfig('EMC_PICKUP_F2') == '') {
        Configuration::updateValue('EMC_PICKUP_F2', '17');
    }
    if (EnvoimoinscherModel::getConfig('EMC_PICKUP_T2') == '') {
        Configuration::updateValue('EMC_PICKUP_T2', '24');
    }
    if (EnvoimoinscherModel::getConfig('EMC_NATURE') == '') {
        Configuration::updateValue('EMC_NATURE', false);
    }
    if (EnvoimoinscherModel::getConfig('EMC_ENV') == '') {
        Configuration::updateValue('EMC_ENV', 'TEST');
    }
    if (EnvoimoinscherModel::getConfig('EMC_TYPE') == '') {
        Configuration::updateValue('EMC_TYPE', 'colis');
    }
    if (EnvoimoinscherModel::getConfig('EMC_ORDER') == '') {
