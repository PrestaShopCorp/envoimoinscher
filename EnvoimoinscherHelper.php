...
class EnvoimoinscherHelper {
	...
	*/
	protected $fields = array(
		...
		'retrait.pointrelais'  => array(
									'type' => 'input',
									'maxlength' => '15',
									'helper' => '<p class="note">Sélectionnez <a href="#" class="action_module">le code du point de proximité</a></p>'
								),
	...
	*/
	public function prepareMandatory($field, $default, $type = 'text', $order_page = false )
	{
		if (array_key_exists($field['code'], $this->fields))
		{
			...
			$field_html = '<label for="field_'.$uniq_id.'">'.ucfirst($field['label']).'</label>';
			return array(
				'label'  => $field_html,
				'field'  => $field_form,
	...
	*/
	...
	{
		if (isset($this->insurance_choices[$key[0]])) return $this->insurance_choices[$key[0]];
		elseif ($order_page) return array();
		return array();
	}
	...
	* @access public
	* @param string $measure Kg or g.
	* @param float $weight Weight to convert.
	* @return float Weight in kg.
	*/
	public static function normalizeToKg($measure, $weight)
	{
		if ($measure != 'g') return $weight;
		$weight = $weight / 1000;
		return round($weight, 2);
	}

	/**
	* Constructs primary key for emc_api_pricing table.
	* @access public
	...
	*/
	...
	/**
	* Some codes contain 'CC' at the end. This method removes it from them and constructs a
	* proper code to use by this module.
	*/
	public static function constructServiceCodeFromApi($code)
	{
		return str_replace('CC', '', $code);
	}

	public static function constructServiceCodeToApi($code)
	{
		return $code.'CC';
	}
