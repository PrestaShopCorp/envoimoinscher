...
<form method="POST" action="{$EMC_link|escape:'htmlall':'UTF-8'}&EMC_tabs=simple_carriers">
	...
	{if sizeof($simpleEconomicCarriers)}
		<fieldset id="EMC_carriers" class="baseForm">
			<legend>{l s='CARRIERS : weight-based pricing' mod='envoimoinscher'}</legend>
			{include file="$familTableTpl" carriersByType=$simpleEconomicCarriers disableServices=$disableServices titleCount=$titleCount carrierControllerUrl=$carrierControllerUrl}
		</fieldset>
	{/if}
